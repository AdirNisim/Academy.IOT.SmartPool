...
cred = credentials.Certificate("HITPOOL.json")
firebase_admin.initialize_app(cred)
# Get a Firestore client instance
db = firestore.client()
# Reference the HITPOOL collection and the document with ID "1"
hitpool_ref = db.collection("HITPOOL")
...
def on_message(client,userdata,msg):
        ...
        if "Troubleshoot" in m_decode:
            doc_ref = hitpool_ref.document("3")
            doc_ref.update({"service_status": 1})
        elif "Lights" in m_decode:
            doc_ref = hitpool_ref.document("2")
            doc_ref.update({"service_status": 1})
        elif "30" in m_decode:
            doc_ref = hitpool_ref.document("0")
            doc_ref.update({"service_status": 1})
