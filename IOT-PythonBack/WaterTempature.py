...
class ConnectionDock(QDockWidget):
    ...
    def __init__(self,mc):
        ...
        self.ePublisherTopic.setText(comm_topic)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.setWindowTitle('Water Temperature')

        # Init QDockWidget objects        
        self.connectionDock = ConnectionDock(self.mc)        
        self.starting_tempeture = 20
        self.down_back = 0
        self.addDockWidget(Qt.TopDockWidgetArea, self.connectionDock)        

    def update_data(self):
        print('Next update')
        temp= self.starting_tempeture - random.randrange(1,10)/10 - self.down_back
        self.starting_tempeture=temp
        hum=74+random.randrange(1,25)/10
        current_data='tempeture sensor: Temperature: '+str(temp)+' Humidity: '+str(hum)
        self.connectionDock.Temperature.setText(str(temp))
        self.connectionDock.Humidity.setText(str(hum))
        if(self.starting_tempeture < water_tempature_limit):
            self.mc.publish_to(comm_topic,"Its too Hot above 15 degrees, Active Pumps")
            self.down_back=-2
        else:
            self.mc.publish_to(comm_topic,current_data)
